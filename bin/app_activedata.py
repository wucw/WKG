...
def _init_conf():
    ...
    try:
        ...
        for opt in ['sleep_time']:
            ...
            logger.info('%s.%s [%s]' % (SELF, opt, config[opt]))
        # boolean
        for opt in ['db_ensend', 'isdistinct']:
...
def inset2db(db, datas):
    logger.info(
        'Start to insert datas to [%s.%s] and Data removal...' %
        (config['db_name'], config['insert_tab']))
    try:
        data_lst = set(datas)
        db.insertmany(
            logger,
...
def dict2list(data_lst):
    ...
    for datas in data_lst:
        ...
        datalst.append(datas['ranking'])
        datalst.append(datas['rankingChange'])
        datalst.append(datas['activeRate'])
        datalst.append(datas['activeChangeRate'])
        datalst.append(datas['coverageRate'])
        datalst.append(datas['coverageChangeRate'])
        datalst.append(datas['dates'])
...
def task():
    ...
    if config['db_ensend']:
        db = ns_db.mysqldb(
            ...
            config['charset'])
        logger.info(
            'Connection the database of [%s] successfully' %
            config['db_name'])
